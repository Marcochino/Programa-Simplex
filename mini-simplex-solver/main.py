...
class MainWindow(QtWidgets.QWidget):
    def __init__(self):
        ...
        self.setWindowTitle("Método Simplex")

        # header styling
        self.title = QtWidgets.QLabel("Método Simplex", self)
        self.title.setFont(font)

        # style number of vars widgets
        self.var_number_lable = QtWidgets.QLabel("Número de variables:", self)
        self.var_number_input = QtWidgets.QSpinBox(self)

        # style number of constraint widget
        self.var_constraint_label = QtWidgets.QLabel("Número de restricciones: ", self)
        self.var_constraint_input = QtWidgets.QSpinBox(self)

        # buttons
        self.generate_button = QtWidgets.QPushButton("Generar", self)
        self.exit_button = QtWidgets.QPushButton("Salir", self)
...
class Inputwidget(QtWidgets.QWidget):
    def __init__(self, n: int, m: int):
        ...
        self.setWindowTitle("Ingreso de Datos - Método Simplex")
        
        # Objective function layout
        func_layout = QtWidgets.QHBoxLayout()
        func_layout.addWidget(QtWidgets.QLabel("Función Objetivo: ", self))
        ...
        # probleme type
        hbox = QtWidgets.QHBoxLayout()
        hbox.addWidget(QtWidgets.QLabel("Naturaleza del problema: ", self))
        self.problem_nature: QtWidgets.QComboBox = QtWidgets.QComboBox(self)
        self.problem_nature.addItems(['Minimización', 'Maximización'])
        hbox.addWidget(self.problem_nature)
        hbox.addStretch()

        # buttons: exit and solve
        button_layout = QtWidgets.QHBoxLayout()
        exit_button = QtWidgets.QPushButton("Salir", self)
        ...
        self.main_layout.addWidget(QtWidgets.QLabel("Matriz"))
    ...
    def verification(self):
        ...
        if not any(self.objective_function):
            alert.setText("Función objetivo")
        ...
        if any(list(filter(lambda x: True if x < 0 else False, self.boundry))):
            ...
            alert.setInformativeText("Todo límite debe ser positivo.\nPuede que quieras multiplicar por -1")
            alert.exec_()
            return False
        return True
...
class SolvingWidget(QtWidgets.QScrollArea):
    def __init__(self, n, m, a, b, constraints, objective_function, nature=False):
        ...
        main_layout.addWidget(QtWidgets.QLabel("Solución"))
        ...
        if phase1:
            ...
            main_layout.addWidget(QtWidgets.QLabel("Fase uno no necesaria !!!"))


        main_layout.addWidget(QtWidgets.QLabel("Fase 2"))
        iteration = 1
    ...
    def _construct_table(self, iteration_table):
        """Crear widget de tabla para la solución"""
        n: int = len(iteration_table[0])
        m: int = len(iteration_table[1])

        table_widget = QtWidgets.QTableWidget(m + 1, n + 1, self.widget)
        table_widget.setHorizontalHeaderLabels(list( iteration_table[0] + ['Restricciones']))
        table_widget.setVerticalHeaderLabels(iteration_table[1]+ ['costo'])
